        ...
        with open("doctors.txt", "r") as file:
            ...
            for line in file:
                ...
                if len(parts) == 6 and parts[1] == doctor_name:
                    doctor = Doctor(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5])
                    table_DR_Info = doctor.table_Dr_Info()
                    head = Doctor(Dr.ID, Dr.Name, Dr.Specialization, Dr.WorkingTime, Dr.Qualification, Dr.RoomNumber)
                    table_DR_Header = head.table_Dr_Info()
                    print(table_DR_Header)
                    print(table_DR_Info)
                    print("\nBack to the previous Menu")
                    found = True
                    return
...
class Patient:
    ...
    def tablePatientInfo(self):
        return f"{self.pid:8}{self.name:20}{self.disease:12}{self.gender:12}{self.age:4}"
        ...
        with open("patients.txt", "r") as file:
            for line in file:
                ...
                if len(parts) == 5:
                    ...
                    head = Patient(Pa.pid, Pa.name, Pa.disease, Pa.gender, Pa.age)
                    table_Pa_Header = head.tablePatientInfo()
                    print(table_info)
                else:
                    print("Invalid format in line:", line)
        ...
        with open("patients.txt", "r") as file:
            ...
            for line in file:
                ...
                if len(parts) == 5 and parts[0] == patient_id:
                    patient = Patient(parts[0], parts[1], parts[2], parts[3], parts[4])
                    table_PA_Info = patient.tablePatientInfo()
                    head = Patient(Pa.pid, Pa.name, Pa.disease, Pa.gender, Pa.age)
                    table_Pa_Header = head.tablePatientInfo()
                    print(table_PA_Info)
                    print("\nBack to the previous Menu")
